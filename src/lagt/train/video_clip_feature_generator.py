...
class VideoClipFeatureGenerator(Sequence):
    def __init__(self, video_mos, vids, batch_size, shuffle=True, clip_length=16, padding='post', ugc_chunk_pickle=None,
        ...
        if 'live' in database or 'konvid' in database or 'ugc' in database:
            self.get_feature_scores(video_mos, vids)
        else:
            self.get_feature_scores(video_mos, None, 'k150k')
    ...
    def get_feature_scores(self, video_mos, vids, dataset='default'):
        """
...
if __name__ == '__main__':
    video_mos_file = r'..\meta_data\all_video_mos.csv'
    ...
    train_vids, test_vids = gather_all_vids(all_vids_pkl=r'..\meta_data\all_vids.pkl')
